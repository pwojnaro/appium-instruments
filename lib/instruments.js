...
Instruments.prototype.start = function (launchCb, unexpectedExitCb) {
  ...
  this.setInstrumentsPath(function (err) {
    ...
    var pre_launched = (typeof process.env.APPTHWACK_MODE !== "undefined");
    this.initSocketServer(function (err) {
      if (pre_launched) {
        logger.info("Skipping launch, AppThwack mode");
...
Instruments.prototype.instrumentsRunning = function (cb) {
  exec('ps | grep instruments', function (err, stdout) {
    if (typeof stdout === "undefined") stdout = "";
    if (stdout.indexOf('instruments') === -1){
      logger.info("Instruments already running");
      cb(true);
    };
    cb(false);
  });
